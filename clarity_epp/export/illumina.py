...
from clarity_epp.export.utils import create_samplesheet, get_sample_sequence_index, reverse_complement
import config
...
def define_project_types_and_set_sample_projects(families, samplesheet_samples):
    """Defines project types and sets projects for samples

    Args:
        families (dict): Dictionary with family information
        samplesheet_samples (dict): Dictionary with samplesheet information

    Returns:
        dict: Updated samplesheet_samples dictionary
    """
    # Get all project types and count samples
    project_types = {}
    for family in families.values():
...
def get_samplesheet_information(sample_artifacts, process, index_2_conversion_orientation):
    """Gets information for filling samplesheet

    Args:
        sample_artifacts (list): List of Lims Artifact objects
        process (object): Lims Process object
        index_2_conversion_orientation (str): Conversion orientation index 2

    Returns:
        dict: Dictionary with samplesheet information
    """
    families = {}
    samplesheet_samples = {}
...
def generate_samplesheet_sequencer(lims, process_id, output_file):
    """Generate BCLConvert samplesheet for NovaSeq sequencer
    ...
    for lane_idx, lane_artifact in output_container.get_placements().items():
        lane_idx = lane_idx.split(':')[0]
        sample_artifacts = get_sample_artifacts_from_pool(lims, lane_artifact)
        samplesheet_samples[lane_idx] = get_samplesheet_information(
            sample_artifacts, process, sequencer_conversion_settings['index_2_conversion_orientation']
        )

    # Add header to SampleSheet
    header = {
        "platform": sequencer_conversion_settings['instrument_platform'],
        "orientation": sequencer_conversion_settings['index_orientation'],
        "run": process.udf['Experiment Name'],
        "read_1_cycles": process.udf['Read 1 Cycles'],
        "read_2_cycles": process.udf['Read 2 Cycles'],
        "read_1_index": process.udf['Index Read 1'],
        "read_2_index": process.udf['Index Read 2'],
        "version": sequencer_conversion_settings['software_version'],
        "compression": sequencer_conversion_settings['fastq_compression_format'],
        "read_1_adapter": process.udf['Adapter'],
        "read_2_adapter": process.udf['Adapter Read 2']
    }
    ...
    for lane, lane_samples in sorted(samplesheet_samples.items()):
        for sample in lane_samples:
            samples[f"{lane}_{sample}"] = {
                "sample": sample,
                "index_1": lane_samples[sample]['index_1'],
                "index_2": lane_samples[sample]['index_2'],
                "cycles": lane_samples[sample]['override_cycles'],
                "project": lane_samples[sample]['project']
            }
            if multiple_lanes:
                samples[f"{lane}_{sample}"]["lane"] = lane

    samplesheet_content = {"header": header, "samples": samples}

    # Create SampleSheet
    if multiple_lanes:
        samplesheet = create_samplesheet("Samplesheet_Illumina_BCLConvert_multi_lane.csv", samplesheet_content)
    else:
        samplesheet = create_samplesheet("Samplesheet_Illumina_BCLConvert.csv", samplesheet_content)
