...
dx_projects = [
    project
    for project in lims.get_projects(udf={"Application": "DX"})
    if project.name.startswith("Dx WL24")
]
sample_count = 0
...
processes = [
    ...
    "Dx Uitvullen en zuiveren (Fluent 480)",
    ...
    "Dx Multiplexen sequence pool",
    "Dx Library pool denatureren en laden (NovaSeq)",
    "AUTOMATED - NovaSeq Run (NovaSeq 6000 v3.1)",
    "Dx Library pool denatureren en laden (NovaSeqXPlus)",
    "Dx NovaSeqXPlus Run",
    "Dx NovaSeq QC controle Lab sequencen",
    "Dx Bioinformatica analyses",
    "Dx NGS onderzoeken afronden",
    "Dx maken mengfractie",
    "Dx Placement Enrichment Magnis",
    "Dx NovaSeqXPlus Run Dx NovaSeq QC controle Lab sequencen",
...
qc_processes = [
    "Dx Qubit QC",
    "Dx Tecan Spark 10M QC",
    "Dx Bioanalyzer QC",
    "Dx Tapestation 2200 QC",
    "Dx Tapestation 4200 QC",
    "Dx Aggregate QC",
]
processes_before_qc = [
    "Dx Sample registratie zuivering",
...
for project in dx_projects:
    for sample in lims.get_samples(projectlimsid=project.id):
        if (
            "Dx Onderzoeksreden" not in sample.udf
            or sample.udf["Dx Onderzoeksreden"] == "Research"
        ):  # skip research
            continue
        sample_count += 1
        sample_quarter = month_quarter[sample.date_received.split("-")[1]]
        for artifact in lims.get_artifacts(
            samplelimsid=sample.id, resolve=True, type="Analyte"
        ):
            for process in lims.get_processes(inputartifactlimsid=artifact.id):
                ...
                if "QC" in process_name and artifact.parent_process:
                    process_name = "{0} {1}".format(
                        re.sub(r" v\d\.\d", "", artifact.parent_process.type.name),
                        process_name,
                    )
                ...
                if (
                    process.all_outputs()
                ):  # outputs_per_input returns TypeError if there is no output
                    output_artifacts = [
                        output_artifact.id
                        for output_artifact in process.outputs_per_input(
                            artifact.id, Analyte=True
                        )
                    ]
                ...
                for action in process.step.actions.get_next_actions():
                    if action["artifact"].id in output_artifacts:
                        process_action_counts[sample_quarter][process_name][
                            "total"
                        ] += 1
                        process_action_counts[sample_quarter][process_name][
                            str(action["action"])
...
for quarter in quarters:
    ...
    for process in process_action_counts[quarter]:
        if process_action_counts[quarter][process]["total"]:
            print(
                "{process}\t{action_list}".format(
                    process=process,
                    action_list="\t".join(
                        [
                            str(process_action_counts[quarter][process][action])
                            if action in process_action_counts[quarter][process]
                            else "0"
                            for action in action_list
                        ]
                    ),
