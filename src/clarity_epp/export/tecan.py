...
from clarity_epp.helpers import get_well_plate_from_process
from clarity_epp.export.utils import sort_96_well_plate, get_well_index

app = typer.Typer(no_args_is_help=True)


@app.command("qc_samplesheet")
...
@app.command("purify_normalise_samplesheet")
def export_purify_normalise_samplesheet(
    process_id: str, output_file: Annotated[typer.FileTextWrite, typer.Argument()] = "/dev/stdout"
) -> None:
    """
    Export Purify & Normalise samplesheet.
    ...
    for well in sort_96_well_plate(well_plate.keys()):
        ...
        output_file.write(f"{sample.udf['Dx Fractienummer']};{well};{get_well_index(well, one_based=True)}\n")
...
#         output_file.write("SourceTubeID;VolSample;VolWater;PositionIndex;MengID\n")
...
#                     samples[monster]["message"] = "Conc. too low - volume= {calc_sample} ul".format(calc_sample=calc_sample)
#                 else:
...
#                 monster = sample.udf["Dx Monsternummer"]
#                 output_file.write(
#                     "{sample};{volume_sample:.2f};{volume_water:.2f};{index};{name};{empty};{message}\n".format(
...
#                 "Dx labels nunc",
#                 "Dx pipetteerschema manueel normaliseren",
#                 "Dx Fluent480 samplesheet manueel normaliseren",
#             ]:
...
#                 output_file.write(
#                     "{sample};{well};{index}\n".format(
#                         sample=source_tube, well=well, index=clarity_epp.export.utils.get_well_index(well, one_based=True)
#                     )
#                 )
